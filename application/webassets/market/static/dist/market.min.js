...
quickView.innerHTML=''
ppd_sum=checkboxPlugin.props.store.state.rowIds.map((rowId)=>parseInt(rowId.split(', ')[11])).reduce((a,b)=>a+b,0).toLocaleString();est_purchase_value=checkboxPlugin.props.store.state.rowIds.map((rowId)=>parseInt(rowId.split(', ')[1])*parseInt(rowId.split(', ')[4])).reduce((a,b)=>a+b,0).toLocaleString();return quickView.innerHTML=`${checkboxPlugin.props.store.state.rowIds.length}\ Items Selected;\ PPD SUM:\ ${ppd_sum}\;\ EST PURCHASE VALUE:\ ${est_purchase_value}`});})}